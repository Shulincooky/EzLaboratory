...
bool ExperimentSidebarConfigLoader::loadFromFile(const QString& filePath)
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        m_errorString = QStringLiteral("Failed to open config file: %1").arg(filePath);
        return false;
    }
...
bool ExperimentSidebarConfigLoader::parseRoot(const QByteArray& jsonData)
{
    ...
    if (parseError.error != QJsonParseError::NoError || !doc.isObject()) {
        m_errorString = QStringLiteral("Failed to parse JSON: %1").arg(parseError.errorString());
        return false;
    }
    ...
    if (!root.contains("chemicals") || !root.value("chemicals").isArray()) {
        m_errorString = QStringLiteral("Config is missing a chemicals array.");
        return false;
    }

    if (!root.contains("sidebar") || !root.value("sidebar").isArray()) {
        m_errorString = QStringLiteral("Config is missing a sidebar array.");
        return false;
    }
    ...
    if (root.contains("reactions")) {
        if (!root.value("reactions").isArray()) {
            m_errorString = QStringLiteral("Config field reactions must be an array.");
            return false;
        }
...
bool ExperimentSidebarConfigLoader::parseChemicals(const QJsonArray& array)
{
    for (const QJsonValue& value : array) {
        if (!value.isObject()) {
            m_errorString = QStringLiteral("Each chemicals entry must be an object.");
            return false;
        }
        ...
        if (id.isEmpty()) {
            m_errorString = QStringLiteral("Chemical entry is missing id.");
            return false;
        }
        ...
        if (chemical.phase != "liquid") {
            m_errorString = QStringLiteral("Chemical %1 uses unsupported phase %2. Only liquid is supported.")
                .arg(id, chemical.phase);
            return false;
        }
...
bool ExperimentSidebarConfigLoader::parseSidebar(const QJsonArray& array)
{
    for (const QJsonValue& value : array) {
        if (!value.isObject()) {
            m_errorString = QStringLiteral("Each sidebar entry must be an object.");
            return false;
        }
        ...
        if (entryType == "beaker") {
            ...
            if (item.displayName.isEmpty()) {
                item.displayName = QStringLiteral("Beaker");
            }
        ...
        if (entryType != "chemical_container") {
            m_errorString = QStringLiteral("Unsupported sidebar.type: %1").arg(entryType);
            return false;
        }

        const QString chemicalId = obj.value("chemicalId").toString().trimmed();
        if (chemicalId.isEmpty()) {
            m_errorString = QStringLiteral("chemical_container entry is missing chemicalId.");
            return false;
        }

        if (!m_chemicals.contains(chemicalId)) {
            m_errorString = QStringLiteral("Sidebar entry references unknown chemicalId: %1").arg(chemicalId);
            return false;
        }

        const ChemicalDefinition chemical = m_chemicals.value(chemicalId);
        const QString requestedContainer = obj.value("container").toString("auto").trimmed().toLower();
        const QString containerType = resolvedContainerType(requestedContainer);
        if (containerType.isEmpty()) {
            m_errorString = QStringLiteral("Chemical %1 uses unsupported container value %2.")
                .arg(chemicalId, requestedContainer);
            return false;
        }
        ...
        const QString displayName = obj.value("displayName").toString().trimmed();
...
bool ExperimentSidebarConfigLoader::parseReactions(const QJsonArray& array)
{
    for (const QJsonValue& value : array) {
        if (!value.isObject()) {
            m_errorString = QStringLiteral("Each reactions entry must be an object.");
            return false;
        }
        ...
        for (const QJsonValue& reactantValue : reactantsArray) {
            ...
            if (!m_chemicals.contains(id)) {
                m_errorString = QStringLiteral("Reaction references unknown reactant: %1").arg(id);
                return false;
            }
        ...
        if (reaction.reactantIds.isEmpty()) {
            m_errorString = QStringLiteral("Reaction is missing reactants.");
            return false;
        }
        ...
        if (!reaction.productLiquidId.isEmpty()) {
            if (!m_chemicals.contains(reaction.productLiquidId)) {
                m_errorString = QStringLiteral("Reaction references unknown liquid product: %1")
                    .arg(reaction.productLiquidId);
                return false;
            }
...
QString ExperimentSidebarConfigLoader::resolvedBottleDisplayName(
    const ChemicalDefinition& chemical,
    const QString& containerType) const
{
    const QString base = resolvedChemicalDisplayName(chemical);
    if (containerType == "wide_bottle") {
        return base + QStringLiteral(" wide bottle");
    }
    if (containerType == "narrow_bottle") {
        return base + QStringLiteral(" narrow bottle");
    }
    return base;
}

QString ExperimentSidebarConfigLoader::resolvedContainerType(const QString& requested) const
{
    ...
    if (requested == "auto" || requested.isEmpty()) {
        return QStringLiteral("narrow_bottle");
    }
